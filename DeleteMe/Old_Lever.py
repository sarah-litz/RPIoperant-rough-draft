...
class Lever(Pin): 
    ...
    def __init__(self, pin_name, pin_number, type=None, gpio_obj=None, lever_angles=None, servo_dict=None, continuous_servo_speed=None): 
        ...
        print("SERVO IS SET TO : ", self.servo)
    ...
    def set_servo_dict_value(self, servo_dict): # option to pass in custom servo dictionary 
        
        if servo_dict is None: 
            # get all values that correspond to self.key 
            return default_operant_settings.servo_dict.get(self.key)            
        else: 
            # return the servo_dict value of corresponding self.key
            result = servo_dict.get(self.key)
            if result is None: 
                print(f'{self.key} does not have a corresponding value in servo_dict :( ')
            else: 
                return servo_dict.get(self.key)
